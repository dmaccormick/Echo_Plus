...
namespace Thesis.VisTrack
{
    ...
    public class VisTrack_Skeletal : MonoBehaviour, IVisualizable
    {
        ...
        public int FindDataPointForTime(float _time)
        {
            ...
            Assert.IsNotNull(m_animDataPoints, "Track Assert Failed [" + GetTrackName() + "] - " + "m_dataPoints has to be setup for before looking for a data point on object [" + this.gameObject.name + "]");
            Assert.IsTrue(m_animDataPoints.Count >= 1, "Track Assert Failed [" + GetTrackName() + "] - " + "m_dataPoints cannot be empty on object [" + this.gameObject.name + "]");

            if (m_animDataPoints.Count > 1)
            {
                // If we are PAST the last data point, return the final datapoint again
                if (_time >= m_animDataPoints[m_animDataPoints.Count - 1].m_timestamp)
                    return m_animDataPoints.Count - 1;
        ...
        private int SearchForward(bool _includeLastIndex, float _time)
        {
            ...
            for (int i = startIndex; i < m_animDataPoints.Count; i++)
            {
        ...
        {
            if (_index >= m_animDataPoints.Count - 1)
                return false;

            // Get the datapoint at the current index and next index
            var thisDataPoint = m_animDataPoints[_index];
            var nextDataPoint = m_animDataPoints[_index + 1];
